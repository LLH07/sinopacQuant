...
class TradeInfo:
    def __init__(self, startDate, endDate, basket):
        self.startDate = startDate
        self.endDate = endDate
        self.tradeBasket = basket

def init(startDate, endDate, basket):
    """
    Initialize a TradeInfo object with start/end dates and basket.
    Usage:
      info = myTrade.init(startDate, endDate, [testCcy])
    """
    return TradeInfo(startDate, endDate, basket)
...
def getDetailedTradingStats(portDailyChange, startDate, endDate, printResult=True):
    ...
    # Optional: collect summary stats for programmatic use (for optimizations)
    return_stats = {
        'Sharpe': yearlyRes.loc['回測期間', 'yearlySharpe'],
        'Mean': yearlyRes.loc['回測期間', 'return(%)'],
        'Std': yearlyRes.loc['回測期間', 'dailyStd(%)'],
        'MDD': yearlyRes.loc['回測期間', 'yearlyMDD(%)'],
    }

    if printResult:
        print(yearlyRes)
    # Always return both DataFrame and stats dict for code use
    return return_stats if not printResult else yearlyRes
