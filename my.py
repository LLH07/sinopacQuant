...
def getTotalSignal(totalIdx, buySignal, sellSignal, px):
    '''
    input buy signals and sell signals and return the total trade signals
    '''
    import numpy as np
    
    test = pd.DataFrame(index=totalIdx)
    test['buySignal'] = buySignal
    test['sellSignal'] = sellSignal
    test['close'] = px
    ...
    test['tradeSignal'] = signals

    return test

def getTradeList(tradeSignal):
    trades = []
    position = 0  # 0 = flat, 1 = long, -1 = short
    entry_price = 0
    entry_date = None
    ...
    for idx, row in tradeSignal.iterrows():
        if row['tradeSignal'] == 1:  # Open long
            position = 1
            entry_price = row['close']
            entry_date = idx
        elif row['tradeSignal'] == -1:  # Open short
            position = -1
            entry_price = row['close']
            entry_date = idx
        elif row['tradeSignal'] == -2 and position == 1:  # Close long
            profit = (row['close'] - entry_price) / entry_price
            holdPeriod = (idx - entry_date).days
            trades.append({'entry_date': entry_date, 'exit_date': idx, 'type': 'long', 'holdPeriod':holdPeriod, 'entry': entry_price, 'exit': row['close'], 'profitPercent': profit})
            position = 0
        elif row['tradeSignal'] == 2 and position == -1:  # Close short
            profit = (entry_price - row['close']) / entry_price
            holdPeriod = (idx - entry_date).days
            trades.append({'entry_date': entry_date, 'exit_date': idx, 'type': 'short', 'holdPeriod':holdPeriod, 'entry': entry_price, 'exit': row['close'], 'profitPercent': profit})
            position = 0
