...
class option():
    def __init__(self, ccy, tenor):
        ...
        self.atmVolCode = f'USD{self.ccy}V{tenor} Curncy' if self.ccy not in self.revL else f'{self.ccy}USDV{tenor} Curncy'
        self.rrVolCode = f'USD{self.ccy}25R{tenor} Curncy' if self.ccy not in self.revL else f'{self.ccy}USD25R{tenor} Curncy'
        self.bfVolCode = f'USD{self.ccy}25B{tenor} Curncy' if self.ccy not in self.revL else f'{self.ccy}USD25B{tenor} Curncy'
    ...
    def getImpliedVol(self, startDate, endDate, DELTA='ATM'): # default is ATM vol
        ...
        if DELTA != 'ATM':
            volCode = f'USD{self.ccy}V{DELTA}{self.volCode[-7:]}' if self.ccy not in self.revL else f'{self.ccy}USDV{DELTA}{self.volCode[-7:]}'
            return blp.bdh(volCode, 'PX_LAST', start_date=startDate, end_date=endDate, Per='D').iloc[:, 0].reindex(dr).ffill().shift(1).dropna()
        
        return blp.bdh(self.atmVolCode, 'PX_LAST', start_date=startDate, end_date=endDate, Per='D').iloc[:, 0].reindex(dr).ffill().shift(1).dropna()
    ...
    def get30minBfixPrice(self, ccy, startDate, endDate):
    ...
    def fetchData(self, startDate, endDate, DELTA=50): # default 50 delta
        from xbbg import blp
        dr = pd.date_range(
            start=pd.to_datetime(startDate),
            end=pd.to_datetime(endDate),
        ...
        # this version if for 25Delta:
        from xbbg import blp
        dr = pd.date_range(start=startDate, end=endDate, freq='B')
        atm_vol = blp.bdh(self.atmVolCode, 'PX_LAST', start_date=startDate, end_date=endDate, Per='D').iloc[:, 0].reindex(dr).ffill()
        rr_vol = blp.bdh(self.rrVolCode, 'PX_LAST', start_date=startDate, end_date=endDate, Per='D').iloc[:, 0].reindex(dr).ffill()
        bf_vol = blp.bdh(self.bfVolCode, 'PX_LAST', start_date=startDate, end_date=endDate, Per='D').iloc[:, 0].reindex(dr).ffill()
        ...
        if DELTA != 50:
            self.callVol = atm_vol + bf_vol + rr_vol / 2
            self.putVol = atm_vol + bf_vol - rr_vol / 2
        else: # atm 
            self.callVol = atm_vol
            self.putVol = atm_vol
        ...
        self.spotBfix1600 = blp.bdh(f'{self.ccyFull} T170 Curncy', "PX_LAST", startDate, endDate).iloc[1:, 0].reindex(dr).ffill() # don't need to shift for one day but delete first date to align with other data
        ...
        self.callVol = self.callVol.shift(1)
        self.putVol = self.putVol.shift(1)
        self.atmVol = self.atmVol.shift(1)
        self.bfVol = self.bfVol.shift(1)
        self.rrVol = self.rrVol.shift(1)

        # remove the first row which is NaN
        # self.vol = self.vol.dropna()
        self.atmVol = self.atmVol.dropna()
        self.callVol = self.callVol.dropna()
        self.putVol = self.putVol.dropna()
        self.bfVol = self.bfVol.dropna()
        self.rrVol = self.rrVol.dropna()
    ...
    def getOptionPrice(self, s, K, F, sigma, expire, delivery, usdRate, foredayBasis, cptydayBasis, strgy, date=None):
        ...
        if strgy == "Call":
            ...
            # if date is not None:
            #     print(f'date: {date}, s: {s}, K: {K}, F_new: {F_new}, sigma: {sigma}, expire: {expire}, delivery: {delivery}, usdRate: {usdRate}, foredayBasis: {foredayBasis}, cptydayBasis: {cptydayBasis}, strgy: {strgy}')
            #     print(f'option Price: {european_option}')
    ...
    # def getDeltaStrikeDf(self, ccy, timeIndex=None, deltas=[10, 25, 50, 75, 90]):
    #     if not timeIndex:
    #         return "Please provide a time index for the delta strike calculation."
    #     deltaDf = pd.DataFrame(index=timeIndex.index)
    #     testCcyFull = f'{ccy}USD' if ccy in ['EUR', 'GBP', 'AUD', 'NZD'] else f'USD{ccy}'
    #     for i in timeIndex.index:
    #         curDate = timeIndex.loc[i, 'enterTime']
    #         deltaDf.loc[i, 'enterTime'] = test.loc[i, 'enterTime']
    #         deltaDf.loc[i, 'atmSpot'] = test.loc[i, 'entry_price']
    #         s = deltaDf.loc[i, 'atmSpot']
    #         F = s + opt.swap.loc[curDate]
    #         usdRate = opt.usdRate.loc[curDate]/100
    #         sigma = opt.vol.loc[curDate] / 100
    #         T = expiry_days


    #         foredayBasis = opt.usdDayCount if opt.ccy not in opt.revL else opt.otherCcyDayCount
    #         cptydayBasis = opt.otherCcyDayCount if opt.ccy not in opt.revL else opt.usdDayCount

    #         # print the above information plz
    #         # print(f"ccy: {opt.ccy}, Date: {curDate}, Spot: {s}, Fwd: {F}, USD Rate: {usdRate}, Sigma: {sigma}, T: {T}, Foreday Basis: {foredayBasis}, Cptyday Basis: {cptydayBasis}")

    #         for d in deltas:
    #             deltaDf.loc[i, f'{d}Dcall'] = opt.getDeltaStrike(testCcyFull, s, F, sigma, T, T, usdRate, foredayBasis, cptydayBasis, d * 0.01, 'Call')
    #             deltaDf.loc[i, f'{d}Dput'] = opt.getDeltaStrike(testCcyFull, s, F, sigma, T, T, usdRate, foredayBasis, cptydayBasis, -1 * d * 0.01, 'Put')
    ...
    def optionProfit_strangle(self, dir, highCallPx, lowPutPx, highCallK=None, lowPutK=None, fixingRate=None):
        dir = dir.upper()
    ...
    def optionProfit_rr(self, dir, highCallPx, lowPutPx, highCallK=None, lowPutK=None, fixingRate=None):
        dir = dir.upper()
        prem = highCallPx - lowPutPx
        if fixingRate and highCallK and lowPutK:
            if fixingRate >= highCallK:
                fixKProfit = fixingRate - highCallK # >0
            elif lowPutK < fixingRate < highCallK:
                fixKProfit = 0
            elif fixingRate <= lowPutK:
                fixKProfit = fixingRate - lowPutK
            netProfit = fixKProfit - prem
    ...
    def optionProfit_spread(self, dir, use, highPx, lowPx, highK=None, lowK=None, fixingRate=None):
        dir = dir.upper()
        use = use.upper()
        
        prem = lowPx - highPx # 不管用甚麼組，看漲spread都是買低K賣高K


        if use == 'CALL': # calculate profit
            if fixingRate and highK and lowK:
                if fixingRate >= highK: # both call 均執行
                    fixKProfit = (highK - fixingRate) + (fixingRate - lowK) 
                elif lowK < fixingRate < highK: # high K 不執行但low K執行
                    fixKProfit = fixingRate - lowK
                elif fixingRate <= lowK: # both call 均不執行
                    fixKProfit = 0
        if use == 'PUT':
            if fixingRate and highK and lowK:
                if fixingRate >= highK: # bot put 均不執行
                    fixKProfit = 0
                elif lowK < fixingRate < highK: # high K 不執行但low K執行
                    fixKProfit = fixingRate - highK
                elif fixingRate <= lowK: # both put 均執行
                    fixKProfit = (lowK - fixingRate) + (fixingRate - highK)
        netProfit = fixKProfit - prem
